                                            ...
                                            if (findData[0].logout == 'done' && findData[0].login == 'notDone') {
                                                careatorMaster.update({ "_id": ObjectId(findData[0]._id), "status": "active" }, { $set: { "password": password, "invite": [], "logout": "notDone", "login": "done" } }, function (err, data) {
                                                    ...
                                                    if (err) {
                                                        ...
                                                        var date = new Date();
                                                        loginDetails.update({ "_id": ObjectId(findData[0]._id) }, { $set: { "userId": findData[0]._id,"orgId":findData[0].orgId,"userName": findData[0].name, "email": findData[0].email, "login": true, "loginDate": date, "logout": false } }, { upsert: true }, function (err, loginData) {
                                                            if (err) {
                                                                responseData = {
                                                                    status: false,
                                                                    message: property.E0007
